'use client'
// src/app/exams/ExamsClient.tsx — Public Exams Listing
...
function Logo({ size = 38 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100">
      <defs>
        <linearGradient id="lg" x1="30" y1="15" x2="70" y2="55" gradientUnits="userSpaceOnUse">
      ...
      <text x="50" y="66" textAnchor="middle" fontFamily="Arial Black,sans-serif" fontWeight="900" fontSize="10.5" fill={G} letterSpacing="1.5">
        ASSAM
      </text>
      <text x="50" y="77" textAnchor="middle" fontFamily="Arial Black,sans-serif" fontWeight="900" fontSize="10.5" fill={W}>
        CAREER
      </text>
      <line x1="22" y1="80" x2="78" y2="80" stroke={T} strokeWidth="0.8" />
      <text x="27" y="90" textAnchor="middle" fontSize="5" fill={T}>
        ◆
      </text>
      <text x="50" y="90" textAnchor="middle" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="8" fill={T} letterSpacing="2">
        POINT
      </text>
      <text x="73" y="90" textAnchor="middle" fontSize="5" fill={T}>
        ◆
      </text>
    </svg>
  )
...
const FALLBACK: Exam[] = [
  {
    ...
    conductedBy: 'CBSE',
    category: 'Teaching',
    description: 'National eligibility test for teachers for Class 1–8.',
    applicationStart: '2026-02-15',
    applicationLastDate: '2026-03-15',
    paymentLastDate: '2026-03-17',
    examDate: '2026-05-22',
    examTime: '9:30 AM – 12:00 PM',
    admitCardDate: '2026-05-10',
    resultDate: '2026-06-30',
    fee: '1 Paper: Gen ₹1,000 | Both: Gen ₹1,200',
    eligibility: 'Graduation + B.Ed / D.El.Ed',
    syllabus: 'Child Dev., Language I & II, Maths / Science / Social',
    officialSite: 'ctet.nic.in',
  ...
  },
  {
    id: 2,
    emoji: '🏥',
    title: 'NEET UG 2026 – Medical Entrance',
    conductedBy: 'NTA',
    category: 'Medical',
    description: 'National Eligibility cum Entrance Test for MBBS, BDS, BAMS.',
    applicationStart: '2026-02-01',
    applicationLastDate: '2026-03-31',
    paymentLastDate: '2026-04-01',
    examDate: '2026-05-04',
    examTime: '2:00 PM – 5:20 PM',
    admitCardDate: '2026-04-20',
    resultDate: '2026-06-14',
    fee: 'Gen ₹1,700 · SC/ST ₹1,000',
    eligibility: '10+2 with PCB (min 50%)',
    syllabus: 'Physics, Chemistry, Biology (Class 11 & 12)',
    officialSite: 'neet.nta.nic.in',
...
export default function ExamsClient({ initialExams }: { initialExams: any[] }) {
  const [exams, setExams] = useState<Exam[]>(
    Array.isArray(initialExams) && initialExams.length > 0 ? initialExams : FALLBACK
  )
  const [cat, setCat] = useState('All')
  const [q, setQ] = useState('')
  const loaded = true // data comes from server, no loading state needed
  ...
  return (
    <>
        ...
        >
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', flexShrink: 0 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: 'Arial Black,sans-serif', fontSize: '.65rem', color: T, letterSpacing: '.12em' }}>
                ◆ POINT ◆
              </div>
            </div>
          </Link>
          <nav style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const }}>
            {NAV_LINKS.map(([l, h]) => (
              <Link key={h} href={h} className={`nav-a${h === '/exams' ? ' on' : ''}`}>
                {l}
              </Link>
            ))}
          </nav>
      ...
      <div
        style={{
          background: `linear-gradient(135deg,${N},#0a3050)`,
      ...
      >
        ...
        <div style={{ maxWidth: 480, margin: '0 auto', position: 'relative' as const }}>
          <input
            ...
            placeholder="Search exams (e.g. CTET, NEET, UPSC...)"
            style={{
              width: '100%',
      ...
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: '22px 20px 60px' }}>
        ...
        <div style={{ fontSize: '.82rem', color: '#5a6a7a', marginBottom: 14, fontWeight: 700 }}>
          {`Showing ${filtered.length} exam${filtered.length !== 1 ? 's' : ''}${cat !== 'All' ? ` in ${cat}` : ''}${
            q ? ` matching "${q}"` : ''
          }`}
        </div>
                ...
                <div style={{ padding: '18px 20px', flex: 1, display: 'flex', flexDirection: 'column' as const, gap: 12 }}>
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const, marginBottom: 5 }}>
                        <span className="pill" style={{ background: `${sc}20`, color: sc, border: `1px solid ${sc}44` }}>
                          ● {exam.status}
                        </span>
                        <span
                          className="pill"
                          style={{ background: '#f0f4f8', color: '#5a6a7a', border: '1px solid #d4e0ec' }}
                        >
                          {exam.category}
                        </span>
                      </div>
                    ...
                    {[
                      ...
                      .map((d) => (
                        <div key={d.l}>
                          ...
                          <div style={{ fontSize: '.78rem', fontWeight: 700, color: d.urgent ? '#ef4444' : N }}>
                            {fmt(d.v)}
                          </div>
                        </div>
                      ))}
                  ...
                  >
                    <div style={{ fontSize: '.76rem', color: '#5a6a7a' }}>
                      <span style={{ fontWeight: 700, color: N }}>Fee:</span> {(exam.fee || '').slice(0, 40)}
                      {(exam.fee || '').length > 40 ? '…' : ''}
                    </div>
                    {dl && (
                      <span
                        style={{
                          background: dl.includes('⚠️') ? '#fde8ea' : '#e8f5e9',
                          color: dl.includes('⚠️') ? '#ef4444' : '#22c55e',
                          border: `1px solid ${dl.includes('⚠️') ? '#f7bcc0' : '#a5d6a7'}`,
                          padding: '3px 9px',
                          borderRadius: 99,
                          fontSize: '.68rem',
                          fontWeight: 800,
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center' as const, padding: '60px 20px', color: '#8fa3b8' }}>
            ...
            <h3 style={{ fontFamily: 'Sora,sans-serif', fontWeight: 700, fontSize: '1.1rem', color: N, marginBottom: 8 }}>
              No exams found
            </h3>
      ...
      <footer style={{ background: N, borderTop: `3px solid ${G}`, padding: '18px', textAlign: 'center' as const }}>
        <div style={{ fontSize: '.72rem', color: 'rgba(255,255,255,.3)' }}>
          ...
          {[
            ...
            <span key={h}>
              <Link href={h} style={{ color: `${G}88`, textDecoration: 'none' }}>
                {l}
              </Link>{' '}
              ·{' '}
            </span>
