...
export default function InfoDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!item) return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column' as const,gap:16,background:'#f0f4f8'}}>
      ...
      <h2 style={{fontFamily:'Sora,sans-serif',fontWeight:700,fontSize:'1.2rem',color:N}}>Article not found</h2>
      <Link href="/information" style={{padding:'10px 22px',borderRadius:99,background:N,color:G,fontWeight:900,textDecoration:'none',fontFamily:'Arial Black,sans-serif',fontSize:'.85rem'}}>← Back to Information</Link>
    </div>
  )
