'use client'
// src/app/information/InformationClient.tsx
...
const FALLBACK: InfoItem[] = [
  {
    id: 1,
    emoji: '🗳️',
    title: 'Voter ID Registration / Correction 2026',
    category: 'Electoral',
    description: 'Register as a new voter or correct existing voter ID details via Form 6/6A/8 online or offline.',
    lastDate: '2026-04-30',
    importantDates: [{ label: 'Last Date to Apply', date: '2026-04-30', time: '11:59 PM' }],
    status: 'Active',
  ...
  },
  {
    id: 2,
    emoji: '🔗',
    title: 'PAN–Aadhaar Linking Deadline 2026',
    category: 'ID & Documents',
    description: 'Link PAN with Aadhaar to avoid PAN becoming inoperative. ₹1,000 penalty applies.',
    lastDate: '2026-06-30',
    importantDates: [{ label: 'Final Deadline', date: '2026-06-30', time: '11:59 PM' }],
    status: 'Active',
    createdAt: new Date().toISOString(),
  },
]

export default function InformationClient({ initialItems }: { initialItems: any[] }) {
  const [items, setItems] = useState<InfoItem[]>(
    Array.isArray(initialItems) && initialItems.length > 0 ? initialItems : FALLBACK
  )
  const [cat, setCat] = useState('All')
  const [q, setQ] = useState('')
  const loaded = true // data comes from server
  ...
  return (
    <>
      ...
      <header style={{ background: N, borderBottom: `2px solid ${G}`, position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 20px rgba(0,0,0,.4)' }}>
        <div style={{ maxWidth: 1180, margin: '0 auto', padding: '10px 20px', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' as const }}>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', flexShrink: 0 }}>
            <Logo size={40} />
            <div>
              <div style={{ fontFamily: 'Arial Black,sans-serif', fontSize: '.78rem' }}>
          ...
          <nav style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const }}>
            {NAV_LINKS.map(([l, h]) => (
              <Link key={h} href={h} className={`nav-a${h === '/information' ? ' on' : ''}`}>
                {l}
              </Link>
            ))}
          </nav>
      ...
      <div style={{ background: `linear-gradient(135deg,${N},#0a3050)`, padding: '32px 20px 26px', textAlign: 'center' as const }}>
        ...
        <h1 style={{ fontFamily: 'Sora,sans-serif', fontWeight: 800, fontSize: 'clamp(1.3rem,3vw,2rem)', color: W, margin: '0 0 8px' }}>
          Government Information
        </h1>
        <p style={{ color: 'rgba(255,255,255,.55)', fontSize: '.9rem', maxWidth: 500, margin: '0 auto 18px' }}>
          Voter ID, PAN-Aadhaar, Government Schemes, Documents & more — step-by-step guides for every citizen
        </p>
                  ...
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const, marginBottom: 5 }}>
                        ...
                        <span style={{ background: '#f0f4f8', color: '#5a6a7a', padding: '2px 9px', borderRadius: 99, fontSize: '.67rem', fontWeight: 700 }}>
                          {item.category}
                        </span>
                      </div>
                      <h2
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center' as const, padding: '60px 20px', color: '#8fa3b8' }}>
            ...
            <button
              ...
              style={{ padding: '9px 20px', borderRadius: 99, background: N, color: G, fontWeight: 900, border: 'none', cursor: 'pointer', fontFamily: 'Arial Black,sans-serif', fontSize: '.82rem' }}
            >
              Clear Filters
            </button>
          </div>
