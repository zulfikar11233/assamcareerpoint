  ...
  if (!post) return (
    <>
      ...
      <div style={{ textAlign: 'center', padding: '80px 20px', fontFamily: 'Nunito,sans-serif' }}>
        ...
        <h1 style={{ fontFamily: 'Sora,sans-serif', color: N, fontSize: '1.4rem' }}>Service not found</h1>
        <Link href={`/${PATH}`} style={{ color: T, fontWeight: 700 }}>← Back to Services</Link>
      </div>
    </>
  )
    ...
    <>
      ...
      <header style={{ background: N, borderBottom: `3px solid ${G}`, padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <nav style={{ display: 'flex', gap: 16 }}>
          {[['/', 'Home'], [`/${PATH}`, 'Services'], ['/announcements', 'Announcements']].map(([href, label]) => (
      ...
      <div style={{ background: W, borderBottom: '1px solid #e8eef4', padding: '10px 20px' }}>
        <div style={{ maxWidth: 860, margin: '0 auto', fontSize: '.78rem', color: '#8fa3b8', display: 'flex', gap: 6 }}>
          ...
          <Link href={`/${PATH}`} style={{ color: '#8fa3b8', textDecoration: 'none' }}>Public Services</Link><span>›</span>
      ...
      <div style={{ background: `linear-gradient(135deg, #1a0a2e 0%, ${N} 100%)`, padding: '32px 20px' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
            <span style={{ padding: '3px 12px', borderRadius: 20, background: '#8e44ad', color: W, fontSize: '.72rem', fontWeight: 800, fontFamily: 'Arial Black,sans-serif' }}>🏛 PUBLIC SERVICE</span>
  ...
  return (
    <div key={imgIdx} style={{
      ...
      />
        ...
        {post.affiliateLink && (
          <div style={{ background: `linear-gradient(135deg, ${G}18, #8e44ad22)`, border: `2px solid #8e44ad44`, borderRadius: 14, padding: '20px 22px', marginTop: 8, textAlign: 'center' }}>
  ...
  <div style={{marginTop:24}}>
    ...
    <div style={{whiteSpace:'pre-line',lineHeight:1.9,color:'#3a5068'}}>
      {(post as any).fullDescription}
        ...
        <div style={{ marginTop: 24 }}>
          <Link href={`/${PATH}`} style={{ color: T, fontWeight: 700, textDecoration: 'none', fontSize: '.88rem' }}>← Back to Public Services</Link>
        </div>
      </div>
