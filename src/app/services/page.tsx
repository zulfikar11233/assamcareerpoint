...
useEffect(() => {
    fetch('/api/data/services', { cache: 'no-store' })
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <>
      ...
      <div style={{ background: `linear-gradient(135deg, #1a0a2e 0%, ${N} 100%)`, padding: '36px 20px 32px', textAlign: 'center' }}>
        ...
        <p style={{ color: '#8fa3b8', fontSize: '.9rem', margin: '0 0 20px', maxWidth: 500, marginLeft: 'auto', marginRight: 'auto' }}>
          Government schemes, public services and citizen information for Assam & Northeast India
        </p>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px 16px 60px' }}>
        {!loaded ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <div style={{ fontWeight: 700, color: N, fontSize: '1.1rem' }}>{search ? 'No results found' : 'No services yet'}</div>
          </div>
        ) : (
          <>
            <div style={{ marginBottom: 16, fontSize: '.82rem', color: '#5a6a7a' }}>Showing <strong>{filtered.length}</strong> public services</div>
              ...
              {filtered.map(post => (
                <Link key={post.id} href={`/${PATH}/${post.slug}`} style={{ textDecoration: 'none' }}>
                  <div className="card" style={{ background: W, borderRadius: 14, padding: '18px 22px', border: '1.5px solid #e8eef4', boxShadow: '0 2px 10px rgba(0,0,0,.05)', display: 'flex', gap: 16 }}>
                    <div style={{ width: 48, height: 48, borderRadius: 12, background: COLOR + '18', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.6rem', flexShrink: 0 }}>{post.emoji}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 4 }}>
                        {post.category && <span style={{ padding: '2px 10px', borderRadius: 20, background: COLOR + '18', color: COLOR, fontSize: '.7rem', fontWeight: 700 }}>{post.category}</span>}
                      ...
                      <div style={{ marginTop: 8, fontSize: '.78rem', color: T, fontWeight: 700 }}>{post.sections.length} section{post.sections.length !== 1 ? 's' : ''} · View service →</div>
                    </div>
                  </div>
                </Link>
              ))}
