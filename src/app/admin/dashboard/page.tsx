...
export default function AdminDashboard() {
  ...
  function saveInfo(e:React.FormEvent) {
    ...
    const dates = infDates.filter(d=>d.label&&d.date).map(d => ({label:d.label,date:d.date,...(d.time?{time:d.time}:{})}))
        if (editInfo) setInfoList(prev => prev.map(x => x.id===editInfo.id ? {...editInfo,...inf,importantDates:dates,sections:infoSections,faqs:infoFaqs} : x))
    else {
      const newId = Date.now()
      const newInfo = {
        id: newId,
...
`)
  ...
  setInf(f => ({
    ...
    slug: f.slug ? f.slug : generateSlug(title, Date.now())
                ...
                {/* ── Section: FAQs ── */}
                <div className="sh">❓ FAQs (shown at the bottom of the page)</div>
                <div style={{background:'#e8f4fd',border:'1px solid #90caf9',borderRadius:9,padding:'9px 14px',marginBottom:12,fontSize:'.78rem',color:'#1a3a5c',lineHeight:1.75}}>
                  Add 2–4 short questions and answers. These help the page show up better in Google search results.
                </div>
                <FaqBuilder faqs={infoFaqs} onChange={setInfoFaqs} />
              </div>
              <div style={{ padding:'14px 24px',borderTop:'1px solid #d4e0ec',display:'flex',justifyContent:'flex-end',gap:10 }}>
