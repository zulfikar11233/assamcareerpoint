...
// ─── Extend OthersPost locally to include imageUrl
type OthersPostWithImage = OthersPost & { imageUrl?: string }
...
// ─── Default blank post (with imageUrl) ───────────────────────────────────────
const blankPost = (type: OthersPost['type']): Omit<OthersPostWithImage, 'id' | 'createdAt'> => ({
...
// ─── Section Builder (unchanged, works as is) ─────────────────────────────────
...
function PostModal({
  ...
  initial: OthersPostWithImage | null
  type: OthersPost['type']
  onSave: (p: OthersPostWithImage) => void
  onClose: () => void
}) {
  const [form, setForm] = useState<Omit<OthersPostWithImage, 'id' | 'createdAt'>>(
    initial
      ? { ...initial }
      : blankPost(type)
  )
  ...
  const handleSave = () => {
    ...
    const post: OthersPostWithImage = {
      ...form,
...
export default function OthersAdmin() {
  ...
  const [posts, setPosts] = useState<OthersPostWithImage[]>([])
  const [showModal, setShowModal] = useState(false)
  const [editPost, setEditPost] = useState<OthersPostWithImage | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/data/${collection}`)
      ...
      .then(data => {
        ...
        else setPosts(getOthersPosts(activeType).map(p => ({ ...p, imageUrl: (p as any).imageUrl || '' })))
      })
      .catch(() => setPosts(getOthersPosts(activeType).map(p => ({ ...p, imageUrl: (p as any).imageUrl || '' }))))
  }, [activeType])

  const save = (updated: OthersPostWithImage[]) => {
    setPosts(updated)
    saveOthersPosts(activeType, updated as any)
    const collection = activeType === 'announcement' ? 'announcements'
      : activeType === 'guide' ? 'guides'
      : 'services'
    fetch(`/api/data/${collection}`, {
  ...
  const handleSave = (post: OthersPostWithImage) => {
